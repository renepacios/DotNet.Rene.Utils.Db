...
namespace Rene.Utils.Db.Builder
{
    ...
    internal static class GenericCommandHandlersConfiguration
    {
        ...
        public static WebApplicationBuilder AddMediatRGenericHandlers(this WebApplicationBuilder builder, DbUtilsOptions options, Assembly assemblyToScan = null)
        {
            ...
            var filterSuperType = typeof(IWithGenericHandler<>); //change this type to select viewmodels with generic commandHandlers
            const string INTERFACE_NAME = "IWithGenericHandler";
            const string PROPERTY_MODEL_TYPE_NAME = "MapFromType";

            //opciones de arranque
            var dbContextType = options.DbContextType;
            var uowType = options.UnitOfWorkType;
            ...
            foreach (var viewModelType in viewModelsTypes)
            {
                ...
                var modelType = mapFromTypeProperty.GetValue(instance, null) as Type;
        ...
        public static IServiceCollection AddMediatRGenericHandlers(this IServiceCollection services, DbUtilsOptions options, Assembly assemblyToScan = null)
        {
            var filterSuperType = typeof(IWithGenericHandler<>); //change this type to select viewmodels with generic commandHandlers
            const string INTERFACE_NAME = "IWithGenericHandler";
            const string PROPERTY_MODEL_TYPE_NAME = "MapFromType";
